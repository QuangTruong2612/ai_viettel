"""Strictly refine ONLY NER types and Assertions, leaving candidates 100% untouched."""
...
def process_ner_assertion(input_text: str, entities: list[dict]) -> tuple[list[dict], int]:
    cleaned = []
    changes_count = 0
    ...
    for ent in entities:
        ...
        # Keep candidates COMPLETELY UNTOUCHED!
        # Do not modify ent["candidates"] under any circumstances.
...
def main():
    ...
    print(f"[INFO] Processing {len(output_files)} output files WITHOUT modifying candidates...")

    total_changes = 0

    for fpath in output_files:
        rec_id = int(fpath.stem)
        input_path = inp_dir / f"{rec_id}.txt"
        if not input_path.exists():
            input_path = inp_dir / f"{rec_id}.json"
        if not input_path.exists():
            continue
        ...
        cleaned, changes = process_ner_assertion(input_text, entities)
        total_changes += changes

        if changes > 0:
            fpath.write_text(json.dumps(cleaned, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"[DONE] Processed NER/Assertions with {total_changes} assertion updates. Candidates remain 100% UNTOUCHED.")
