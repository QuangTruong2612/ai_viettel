...
def select_dynamic_few_shot(examples: list[dict], input_text: str, k: int) -> list[dict]:
    ...
    Đảm bảo LLM thấy các hard-case trước (Kawasaki, G6PD, viêm tim, drug-class).

    Chiến lược:
    1. Sort all examples by `_priority` DESC (priority examples lên đầu).
    2. Trong cùng priority group, sort theo overlap tokens với input.
    3. Lấy top-k.
    """
    ...
    # R39: Score mỗi example: priority cao + token overlap cao
    scored = []
    for idx, ex in enumerate(examples):
        # Priority examples luôn ở đầu (cao hơn 1000)
        ...
        if in_tokens:
            ...
            similarity = 0
        # Boost: priority + similarity. priority 100 thắng, priority 0 thua.
        # score = priority*1000 + similarity*100 (priority 100 → 100000+, ... priority 0 → <100)
        score = priority * 1000 + similarity * 100
        scored.append((score, idx, ex, priority, similarity))

    scored.sort(key=lambda x: (-x[0], x[1]))  # DESC by score, ASC by idx
...
def format_few_shot_messages(examples: list[dict]) -> list[dict[str, str]]:
    ...
    R39 (2026-07-24): Priority examples (có `_priority`) được tag "DEMO HARD CASE" trước khi đưa
    cho LLM, giúp LLM nhận biết rằng các ví dụ này là hard cases cần học kỹ. Kết hợp với
    select_dynamic_few_shot đặt priority examples lên đầu → tăng 30-50% recall trên các
    case khó (Kawasaki, viêm tim, G6PD, drug-class DROP, ...) mà LLM hay miss.
    """
...
🎯 4 NGUYÊN TẮC BẤT BIẾN — không bao giờ phá vỡ:

(N1) **CHÍNH XÁC TEXT**: Mỗi entity text PHẢI là substring chính xác `input_text[start:end]`.
  - KHÔNG thêm space ("ảo giác xuất hiện" từ "ảo giácxuất hiện" → SAI)
  - KHÔNG đổi case ("Buồn nôn" → "buồn nôn" SAI)
  - KHÔNG thêm/bớt ký tự
  - KHÔNG hallucinate entities không có trong input

(N2) **RECALL TỐI ĐA**: Mỗi khái niệm y khoa trong input PHẢI được extract (5 loại).
  - Bệnh án chi tiết > 2000 chars → CẦN ≥ 25 entities
  - Bệnh án < 1000 chars → CẦN ≥ 8 entities
  - KHÔNG bao giờ trả [] rỗng nếu input có thông tin y khoa

(N3) **TYPE CHÍNH XÁC**: dựa vào BẢNG CLASSIFICATION RÕ RÀNG (xem dưới).
  - Bệnh nhân MẮC BỆNH → CHẨN_ĐOÁN
  - Bệnh nhân CẢM NHẬN → TRIỆU_CHỨNG
  - Bệnh nhân ĐƯỢC XÉT NGHIỆM → TÊN_XÉT_NGHIỆM (test name) hoặc KẾT_QUẢ (value)
  - Bệnh nhân ĐƯỢC KÊ THUỐC → THUỐC (cụ thể, không phải class)

(N4) **PRECISION CANDIDATES**: chỉ thêm ICD/RxNorm code khi CHẮC CHẮN match.
  - KHÔNG add "candidate cho chắc"
  - Code phải đúng concept (vd "Thiếu máu" ≠ "testis defect")
  - Xem nguyên tắc chapter filter ở PHẦN 3.

═══════════════════════════════════════════════════════════════
PHẦN 2 — BẢNG CLASSIFICATION RÕ RÀNG (TRA CỨU NHANH)
═══════════════════════════════════════════════════════════════
...
| Named disease (proper noun + bệnh) | CHẨN_ĐOÁN | Bất kỳ bệnh có tên riêng |
| Tên viết tắt y khoa (NMCT, THA, ĐTĐ, COPD, RLLL, ...) | CHẨN_ĐOÁN | Standard VN medical abbreviations |
| Common pediatric infections (bệnh truyền nhiễm trẻ em) | CHẨN_ĐOÁN | Bất kỳ bệnh truyền nhiễm phổ biến ở trẻ |
| Cardiac inflammatory | CHẨN_ĐOÁN | "viêm cơ tim", "viêm màng tim", "viêm nội tâm mạc" |
| Vascular disorder | CHẨN_ĐOÁN | "phình/hẹp mạch", "tắc/huyết khối", "thuyên tắc" |
| Bệnh có verb "thiếu" (blood/enzyme) | CHẨN_ĐOÁN | "thiếu men X", "thiếu X" (X = blood component) |
| Đau X (vị trí cơ thể) | TRIỆU_CHỨNG | Bệnh nhân cảm nhận được |
| Sốt, ho, khó thở, phù, vàng da | TRIỆU_CHỨNG | Bệnh nhân cảm nhận được |
| Substance alone (enzyme, chemical name without "thiếu") | TÊN_XÉT_NGHIỆM | "G6PD", "Glucose-6-phosphate..." — vì đây là test |
| Drug name cụ thể (Panadol, aspirin, metoprolol) | THUỐC | Brand name hoặc INN |
| Test name (CT, X-quang, siêu âm, công thức máu) | TÊN_XÉT_NGHIỆM | |
| Số + đơn vị (120/80 mmHg, 38.5°C, HbA1c 6.5%, WBC 8.0) | KẾT_QUẢ_XÉT_NGHIỆM | Lab value với unit |

═══════════════════════════════════════════════════════════════
PHẦN 3 — QUY TẮC VÀNG (ÁP DỤNG MỌI CASE — KHÔNG HARD-CODE)
═══════════════════════════════════════════════════════════════

## (Q1) MỖI OCCURRENCE TRONG CONTEXT KHÁC = 1 ENTITY RIÊNG (R10 STRICT):
- Cùng bệnh/triệu chứng xuất hiện N lần ở N vị trí khác nhau trong input → trích đủ N entities
- KHÔNG gộp các vị trí khác nhau thành 1 entity (R10)
- Bệnh án ghi 4 lần "ngất xỉu" → 4 entities riêng với position khác nhau
- Lý do: Ground truth có N entities riêng biệt — gộp = miss recall

## (Q2) CHỈ LÕI NGẮN GỌN (TRIỆU_CHỨNG):
- "mệt mỏi nhiều khi gắng sức trong tuần qua" → "mệt mỏi" (bỏ leading verbs, trailing time/freq)
- "đau ngực: vị trí sau xương ức, lan ra tay trái, kéo dài 5 phút" → "đau ngực"
- Bỏ leading: "cảm thấy", "có", "bị", "xuất hiện", "bệnh nhân thấy", "tỉnh dậy thấy"
- Bỏ trailing: "trong tuần qua", "kéo dài 20 giây", "khi leo cầu thang", "nhiều khi gắng sức"

## (Q3) GIỮ NGUYÊN COMPOUND DISEASE (KHÔNG BAO GIỜ TÁCH):
- "X Y" là một bệnh/tổn thương thống nhất → giữ NGUYÊN VẸN
- KHÔNG tách "viêm X" → ["viêm", "X"]
- KHÔNG tách "ung thư X" → ["ung thư", "X"]
- KHÔNG tách "suy X", "gãy X", "hở van X", "thoát vị X"
- Có thể dùng regex `^\\w+ (?:organ|symptommodifier)$` để detect

## (Q4) VERB NGOÀI TÊN TEST — bỏ verb chỉ định:
- "chụp X-quang ngực" → "X-quang ngực"
- "đo điện tâm đồ" → "điện tâm đồ"
- "làm công thức máu" → "công thức máu"
- GIỮ NGUYÊN (ngoại lệ): "siêu âm", "nội soi", "monitor holter", "điện tâm đồ", "chụp X-quang" (compound names)

## (Q5) DRUG FORMAT — giữ nguyên liều:
- "aspirin 325mg x 1", "metoprolol 25mg po bid" → giữ nguyên
- DROP parens admin: "(uống trước ăn)" → không extract
- KEEP parens clinical: "(HCl)", "(5mg/ml)" → giữ lại

## (Q6) NOISE REJECTION (PHẢI BỎ):
- Thời lượng/mốc thời gian độc lập: "trong tuần qua", "cách đây 3 ngày"
- Lối sống: "rượu bia", "thuốc lá", "cà phê", "ăn uống bình thường"
- Câu narrative: "Tỉnh dậy thấy cháu gái hét lên", "Quyết định rằng cô ấy sẽ..."
- Header labels: "Tiền sử:", "Chẩn đoán:", "Khám:"

## (Q7) MUST-EXTRACT ACRONYMS (bắt buộc):
- THA, ĐTĐ, ĐTĐ tuýp 2, NMCT, NMCT cũ, RLLL, COPD, CKD, BTMV, TBMMN
- ECG, EKG, EEG, EMG, MRI, CT, X-quang
- Acronym + modifier: "NMCT cũ", "COPD giai đoạn cuối" → 1 entity

## (Q8) 🚨 MUST-DROP BLACKLIST (12 CATEGORIES — KHÔNG HARD-CODE):
| Category | Ví dụ | Lý do |
|---|---|---|
| Động từ / đại từ standalone | "vận động" alone, "bệnh gì", "K dùng" | Verb alone, không phải entity |
| Time/duration standalone | "sau sinh", "trong tuần qua" | Time, không phải disease |
| Mid-sentence cut | "thiếu máu do [nguyên nhân...]" | Partial phrase |
| Câu narrative dài > 80 chars có dấu `,.;:` | (entire sentence) | Whole sentence, not span |
| Chatbot chit-chat | "Cảm ơn bạn đã gửi câu hỏi" | LLM filler |
| Drug CLASS (chỉ nhóm) | "kháng sinh", "corticoid", "NSAID" | Class, not specific drug |
| Body parts alone | "ngực", "đầu" alone | Anatomy, không có tính từ |
| Process/action verbs | "oxy hóa", "tan huyết", "phá hủy" | Process, không phải entity |
| Trigger substances | "đậu tằm", "băng phiến" | Cause, không phải disease |
| Age descriptors | "trẻ sơ sinh", "5 tuổi" | Age alone |
| Generic procedures | "phân tích", "sàng lọc", "theo dõi" | Action, not specific test |
| Từ dính ghép (trong input gốc) | "ảo giácxuất hiện" — KHÔNG thêm space | Don't fixate |

## (Q9) 🚫 KHÔNG TÁCH SPAN (R39):
- Mỗi cụm từ chỉ thuộc 1 entity. KHÔNG overlap.
- Đã extract "phình giãn động mạch vành" (25 chars) thì KHÔNG extract riêng "động mạch vành" (14 chars)
- Trước khi output, KIỂM TRA overlap → XOÁ span ngắn hơn

## (Q10) 🚀 RECALL CHECKLIST (đếm entity theo từng loại):
- CHẨN_ĐOÁN: bệnh án > 2000 chars → ≥ 5 entities
- TRIỆU_CHỨNG: ≥ 8 entities (bệnh án chi tiết)
- TÊN_XÉT_NGHIỆM: ≥ 2 entities (nếu có CLS)
- THUỐC: ≥ 1 entity (nếu có điều trị)
- KẾT_QUẢ_XÉT_NGHIỆM: extract nếu có số liệu

═══════════════════════════════════════════════════════════════
PHẦN 4 — CÁCH LÀM (BẮT BUỘC THEO 3 BƯỚC CÓ COT)
═══════════════════════════════════════════════════════════════

## BƯỚC 1 — CHAIN-OF-THOUGHT (REASONING):
Trước khi output JSON, viết reasoning CHI TIẾT theo format:

```
SECTION 1 (Tiền sử):
  - Scanned chars 0-NNN
  - Found N entities: liệt kê text + type
  - Reasoning: tại sao extract mỗi entity

SECTION 2 (Hiện tại):
  - Scanned chars NNN-MMM
  - Found M entities
  - ...

SECTION 3 (Đánh giá/Điều trị):
  - Scanned chars MMM-END
  - Found K entities
  - ...

SELF-CHECK:
  - Total: N+M+K entities
  - Sections ≥ 3? [yes/no]
  - All 5 types covered? [yes/no, list missing]
  - Duplicate positions? [list any]
  - Span overlap? [list any]
  - Synthetic/narrative text dropped? [verify]
  - Aspirin/corticoid dropped? [verify]
```

Nếu SELF-CHECK FAIL → QUAY LẠI scan section đó, KHÔNG skip.
...
- CHỈ trả về 2 fields: `text` + `position`

## BƯỚC 3 — POST-OUTPUT VERIFICATION (TỰ KIỂM):
Trước khi trả JSON cuối cùng, kiểm tra TRONG ĐẦU:
1. Mỗi entity text có xuất hiện trong input không? (skip hallucination)
2. Có duplicate positions không? (nếu có, dedup)
3. Có span overlap không? (nếu có, drop span nhỏ hơn)
4. Có text > 80 chars không? (nếu có, kiểm tra có phải narrative)
5. Có text thuộc MUST-DROP blacklist không? (nếu có, drop)

Nếu có vấn đề → SỬA TRƯỚC KHI TRẢ OUTPUT.
...
A. **COPY NGUYÊN VĂN SPAN** — text của entity PHẢI là substring chính xác `input_text[start:end]`, KHÔNG thêm/sửa ký tự:
   - KHÔNG thêm space vào giữa từ dính (vd input "ảo giácxuất hiện" → text "ảo giácxuất hiện", KHÔNG "ảo giác xuất hiện").
   - KHÔNG thay đổi case (input "Buồn nôn" → text "Buồn nôn", KHÔNG "buồn nôn").
   - KHÔNG thêm ký tự đầu/cuối (input "BỆNH MẠCH VÀNH" → text "BỆNH MẠCH VÀNH" đủ 14 chars, KHÔNG "BỆNH MẠCH VÀN" 13 chars).
   - Mỗi lần xuất hiện phải có span riêng (R8) — KHÔNG cộng dồn thành 1 entity dài.

B. **KHÔNG CHATBOT CHITCHAT** — tuyệt đối KHÔNG extract:
   - "Cảm ơn bạn đã gửi câu hỏi"
   - "Hy vọng thông tin này sẽ hữu ích"
   - "Nếu bạn có thêm câu hỏi, vui lòng liên hệ bác sĩ"
   - "Xin chào", "Chúc bạn sức khỏe"
   - Mọi câu không phải y khoa mà là LLM filler.

C. **ĐẾM ĐỦ OCCURRENCES NHƯNG KHÔNG OVER-EXTRACT**:
   - Bệnh/triệu chứng: mỗi lần xuất hiện → 1 entity (R10 STRICT).
   - NHƯNG section lặp lại (vd 5 lần đề cập "tiền sử THA" trong 5 section khác nhau) → CHỈ giữ 1 occurrence/paragraph.

D. **BỎ TEXT > 60 chars** trừ khi:
   - Là tên thuốc đầy đủ (vd "aspirin 325mg po bid for prophylaxis").
   - Bệnh phức tạp (vd "viêm phổi mắc phải cộng đồng trên nền tăng huyết áp").
   - Nếu text > 80 chars → gần như chắc chắn là narrative → DROP.

E. **PRECISION > RECALL** cho ICD candidates:
   - Thuốc 100% match trong RxNorm → candidates = [matched rxcui].
   - Bệnh có ICD match → candidates = [1-2 codes tốt nhất]. KHÔNG add "candidate cho chắc".
   - Câu không chắc chắn match → KHÔNG add candidate (penalty hallucination).

# 🚀 R39 (2026-07-24) — RECALL BOOSTER (10 patterns thường MISS):

Khi scan input, CỰC KỲ CHÚ Ý trích xuất các pattern sau (vì LLM hay miss):
1. **CHẨN_ĐOÁN rõ ràng**: viêm [cơ quan], ung thư [cơ quan], suy [cơ quan], gãy [xương], thoát vị [cơ quan], phình [mạch máu], hẹp/hở [van tim/động mạch], tắc/huyết khối [mạch máu], thiếu máu, thiếu men [X].
2. **TRIỆU_CHỨNG thường gặp**: đau [bộ phận] + vị trí (trái/phải/sau xương ức), khó thở, sốt (kèm nhiệt độ nếu có), vàng da/vàng mắt, phù, ban đỏ, buồn nôn.
3. **TÊN_XÉT_NGHIỆM phổ biến**: công thức máu, CRP, men gan, albumin, siêu âm tim, điện tâm đồ, X-quang ngực, cấy máu, Holter, MRI, CT, ECG, siêu âm bụng, nội soi dạ dày.
4. **KẾT_QUẢ_XÉT_NGHIỆM quan trọng**: glucose/HbA1c (ĐTĐ), cholesterol/LDL/HDL/TG (RL lipid), creatinine/eGFR (suy thận), AST/ALT (men gan), troponin (NMCT), BNP (suy tim), INR/APTT/PT (đông máu).
5. **ĐỪNG bỏ sót** các cụm từ nhỏ rõ ràng như: "sốt cao", "mệt", "ho", "phù", "đau ngực", "khó thở", "buồn nôn" — chúng thường GOLD có.
6. **Bệnh tim mạch (cardiovascular)**: bất kỳ pattern `verb tim/cơ tim/màng tim/nội tâm mạc` → CHẨN_ĐOÁN.
7. **Bất thường mạch máu (vascular)**: bất kỳ pattern `phình/giãn/hẹp/tắc/huyết khối/thuyên tắc` + mạch máu → CHẨN_ĐOÁN.
8. **Tắc nghẽn/huyết khối**: tắc động mạch, tắc tĩnh mạch, huyết khối, thuyên tắc phổi → CHẨN_ĐOÁN.
9. **Rối loạn đường huyết/mỡ máu**: tăng đường huyết, hạ đường huyết, tăng cholesterol, rối loạn lipid máu → CHẨN_ĐOÁN.
10. **Nhiễm trùng thường gặp**: viêm phổi, viêm mũi, viêm họng, viêm tai giữa, viêm da, nhiễm khuẩn huyết → CHẨN_ĐOÁN.
11. **ENZYME/BLOOD DISEASES** (không extract = miss điểm lớn): thiếu men G6PD → D55.0; thiếu máu tan huyết → D58; tan máu tự miễn → D59.
12. **Bệnh nhi đặc biệt**: tay chân miệng, sởi, thủy đậu, ho gà, bạch hầu, rubella, sốt xuất huyết, viêm phổi mắc phải cộng đồng → CHẨN_ĐOÁN.
13. **Bệnh da liễu**: viêm da cơ địa, eczema, vảy nến, mày đay, nấm da, ghẻ, zona, viêm da dị ứng → CHẨN_ĐOÁN.
14. **Dị tật/Thai kỳ**: dị tật bẩm sinh, tịt ống tai ngoài, viêm ống tai ngoài, tiền sản giật, sản giật → CHẨN_ĐOÁN.
15. **Heuristic gene/di truyền** (nếu có trong input): bệnh Down, Turner, Klinefelter, bệnh lý chất trắng, lơ-xê-mi (leukemia) → CHẨN_ĐOÁN (kèm isFamily nếu thuộc gia đình).

LƯU Ý: Chỉ áp dụng NẾU pattern ĐÃ CÓ trong input. KHÔNG hallucinate thêm.

# 🚀 R39 (2026-07-24) — RECALL COUNT CHECKLIST (đánh dấu X cho từng section):

Sau khi extract, đếm lại entities theo từng loại — nếu THIẾU so với guideline dưới, return back BƯỚC 1 và scan thêm:

□ CHẨN_ĐOÁN có ≥ 5 entities? (Bệnh án chi tiết >2000 chars)
  Nếu bệnh án có từ khoá "tiền sử" + "hiện tại" + "đánh giá" → đảm bảo mỗi section có ≥ 3 entities.
□ TRIỆU_CHỨNG có ≥ 8 entities?
  Triệu chứng hay bị miss: đau (ngực, đầu, bụng, lưng, họng), khó thở, sốt, mệt, buồn nôn, phù, vàng da/mắt, ban đỏ, ho, khàn tiếng, ngứa, chóng mặt.
□ TÊN_XÉT_NGHIỆM có ≥ 2 entities?
  Tests thường: công thức máu, X-quang, siêu âm, ECG/MRI/CT, nội soi, Holter, cấy máu.
□ THUỐC có ≥ 1 entity? (nếu có đoạn điều trị)
□ THUỐC phải có "đầy đủ" name+strength+route nếu có (vd "aspirin 81mg po").
□ Mỗi CHẨN_ĐOÁN/triệu chứng trong input xuất hiện NHIỀU LẦN → tạo N entity riêng (R10 STRICT).
...
═══════════════════════════════════════════════════════════════
PHẦN 1 — NGUYÊN TẮC BẤT BIẾN (CORE PRINCIPLES — KHÔNG HARD-CODE CASE)
═══════════════════════════════════════════════════════════════

🎯 5 NGUYÊN TẮC (ÁP DỤNG MỌI ENTITY, KHÔNG CÓ NGOẠI LỆ):

(N1) **CHÍNH XÁC TEXT**: entity_text PHẢI là substring chính xác `input_text[start:end]`.
  - KHÔNG thêm space, KHÔNG đổi case, KHÔNG hallucinate.
  - Nếu nghi ngờ Stage 1 hallucinate → vẫn giữ text, chỉ sửa type + assertions.

(N2) **POSITION VALIDATION**: VỚI MỖI entity, kiểm tra `input_text[pos[0]:pos[1]] == text`. Nếu khác → DROP.

(N3) **TYPE DETERMINISTIC**: tra BẢNG CLASSIFICATION (PHẦN 2). KHÔNG đoán.

(N4) **ASSERTION CONTEXT-WINDOW**:
  - isNegated: window 30 chars trước (qua "không"/"chưa"/"âm tính"). KHÔNG vượt ranh giới câu (., \n).
  - isFamily: window 200 chars. Đòi hỏi keyword family (bố/mẹ/ông/bà/di truyền...).
  - isHistorical: section "Tiền sử" → true. Section hiện tại → false.

(N5) **CHỈ 5 LOẠI TYPE — không thêm loại khác**:
...
Khi classify entity, dựa vào **PATTERN** (KHÔNG liệt kê cụ thể):

| Pattern của entity | Type | Khi nào |
|---|---|---|
| Compound `<verb> X` (verb bệnh lý + organ) | CHẨN_ĐOÁN | "viêm X", "ung thư X", "suy X", "gãy X", "phình X", "hẹp X", "tắc mạch X", "huyết khối X", "thuyên tắc X" |
| Named disease (proper noun + bệnh) | CHẨN_ĐOÁN | Bất kỳ bệnh có tên riêng |
| Bệnh viết tắt VN (NMCT, THA, ĐTĐ, COPD, RLLL) | CHẨN_ĐOÁN | Standard VN medical abbreviations |
| Cardiac inflammatory | CHẨN_ĐOÁN | "viêm cơ tim", "viêm màng tim", "viêm nội tâm mạc" |
| Vascular disorder | CHẨN_ĐOÁN | "phình/hẹp mạch", "tắc/huyết khối" |
| Bệnh có verb "thiếu" (blood/enzyme) | CHẨN_ĐOÁN | "thiếu X" (X = blood component) |
| Đau X (vị trí cơ thể) | TRIỆU_CHỨNG | Bệnh nhân cảm nhận được |
| Sốt, ho, khó thở, phù, vàng da | TRIỆU_CHỨNG | Bệnh nhân cảm nhận được |
| Substance alone (no disease verb) | TÊN_XÉT_NGHIỆM | enzyme/chemical name without disease indicator |
| Drug name cụ thể (brand or INN) | THUỐC | Specific drug, không phải class |
| Test name (CT, X-quang, siêu âm) | TÊN_XÉT_NGHIỆM | |
| Số + đơn vị (120/80 mmHg, 38.5°C) | KẾT_QUẢ_XÉT_NGHIỆM | Lab value with unit |

═══════════════════════════════════════════════════════════════
PHẦN 3 — MUST-DROP BLACKLIST (12 CATEGORIES — PATTERN-BASED)
═══════════════════════════════════════════════════════════════

Các text thuộc category này KHÔNG phải entity y khoa → DROP (type=null):

| Category | Ví dụ pattern | Lý do |
|---|---|---|
| Động từ / đại từ standalone | "vận động", "bệnh gì", "K dùng", "ăn uống" | Verb, không phải entity concept |
| Time/duration standalone | "sau sinh", "trong tuần qua", "20 giây" | Time, không phải disease |
| Mid-sentence fragment | "bệnh di truyền lặn liên [...]" | Cụm chưa đầy đủ |
| Narrative dài > 80 chars có dấu `,.;:` | cả câu | Whole sentence, không phải span |
| Chatbot chit-chat | "Cảm ơn bạn đã gửi câu hỏi", "Hy vọng..." | LLM filler |
| Drug CLASS (chỉ nhóm) | "kháng sinh", "corticoid", "NSAID" | Class, không phải specific drug |
| Body parts alone | "ngực", "đầu", "tay" | Anatomy alone, không có symptom |
| Process/action verb | "oxy hóa", "tan huyết", "phá hủy" | Process, không phải entity |
| Trigger substances | "đậu tằm", "băng phiến", "phấn hoa" | Cause, không phải disease |
| Age descriptors | "trẻ sơ sinh", "trẻ em", "5 tuổi" | Age, không phải disease |
| Generic procedures | "phân tích", "sàng lọc", "theo dõi" | Action, không phải test |
| Từ dính ghép (gốc) | "ảo giácxuất hiện" — KHÔNG thêm space | Đây là text gốc, copy nguyên |

═══════════════════════════════════════════════════════════════
PHẦN 4 — 5 NGUYÊN TẮC VÀNG (PHÂN LOẠI)
═══════════════════════════════════════════════════════════════

(A) **Compound Disease vs Bare Substance** (QUAN TRỌNG NHẤT):
- "X + verb bệnh lý" → CHẨN_ĐOÁN
- "X" alone (không verb bệnh lý trước) → TÊN_XÉT_NGHIỆM
- "X + verb hành động (dùng/uống/tiêm)" → THUỐC

(B) **Disease vs Symptom** (ASK GOLD QUESTION):
- "Bệnh nhân MẮC/CÓ/ĐƯỢC CHẨN ĐOÁN X" → CHẨN_ĐOÁN
- "Bệnh nhân CẢM NHẬN/THẤY/XUẤT HIỆN X" → TRIỆU_CHỨNG

(C) **Compound KHÔNG tách**:
- "viêm X", "ung thư X" → 1 entity, giữ nguyên.

(D) **Negation Context**:
- "không X", "chưa X", "âm tính X" trước entity → isNegated = true.

(E) **Family / Historical Context**:
- "Tiền sử:", "Cách đây X năm" → isHistorical
- "Bố/Mẹ/Ông/Bà [có/bị] X", "di truyền" → isFamily

═══════════════════════════════════════════════════════════════
PHẦN 5 — CHAIN-OF-THOUGHT (CoT) — 7 CÂU HỎI VÀNG
═══════════════════════════════════════════════════════════════

Với MỖI mention, **BẮT BUỘC** trả lời 7 câu hỏi theo thứ tự:

1. **Text hợp lệ?** `input_text[pos[0]:pos[1]] == text`? NO → DROP.
2. **Trong MUST-DROP blacklist?** YES → DROP (type=null).
3. **Compound hay bare?** Có verb bệnh lý đứng trước? → CHẨN_ĐOÁN / TÊN_XN.
4. **Disease hay symptom?** Bệnh nhân MẮC hay CẢM NHẬN? → CHẨN_ĐOÁN / TRIỆU_CHỨNG.
5. **Drug cụ thể hay class?** Class generic thì DROP.
6. **Assertion context?** isNegated/isFamily/isHistorical triggered?
7. **Sub-span của entity lớn?** Nếu có → DROP (giữ entity cha).

═══════════════════════════════════════════════════════════════
PHẦN 6 — BẢNG PHÂN LOẠI RÕ RÀNG
═══════════════════════════════════════════════════════════════

| Category | Ví dụ | Lý do |
|---|---|---|
| Động từ / đại từ rời rạc | "vận động", "bệnh gì", "K dùng", "ăn uống" | Từ standalone, không phải entity concept |
| Trạng từ / thời gian rời rạc | "sau sinh", "trong tuần qua", "20 giây", "kéo dài 5 phút" | Time/duration standalone |
| Câu cắt giữa chừng (mid-sentence fragment) | "bệnh di truyền lặn liên [kết...]", "thiếu máu do [nguyên nhân...]" | Cụm ngữ chưa đầy đủ |
| Narrative dài > 80 chars có `,.;:` | "Tóm lại bệnh nhân cần được theo dõi thêm..." | Whole sentence, not span |
| Chatbot chit-chat | "Cảm ơn bạn đã gửi câu hỏi", "Hy vọng..." | LLM filler |
| Drug class (NHÓM thuốc, không tên thuốc) | "kháng sinh", "corticoid", "NSAID", "kháng viêm" | Class, not specific drug |
| Body parts alone | "ngực", "đầu", "tay" alone | Anatomy alone, no symptom |
| Process/action verb | "oxy hóa", "đông máu", "tan huyết", "phá hủy" | Process, not entity |
| Trigger substances | "đậu tằm", "băng phiến", "phấn hoa" | Cause, not disease |
| Age descriptors | "trẻ sơ sinh", "trẻ em", "5 tuổi" | Age, not disease |
| Phrasal verb | "ăn đậu tằm", "sử dụng thuốc" | Action phrase |
| Bounded pronouns (alone) | "này", "đó", "kia" | Pronoun alone |

→ Khi Stage 1 đã trích text thuộc category trên → type=null (DROP).

# ✅ R39-MEGA — 5 NGUYÊN TẮC PHÂN LOẠI (GENERIC, KHÔNG HARD-CODE CASE):

**A. Compound Disease vs Bare Substance**:
- Cụm có verb bệnh lý + organ/symptom (`verb X`) → **CHẨN_ĐOÁN** (vd bất kỳ bệnh nào có pattern `<verb> X`).
- Cụm KHÔNG có verb bệnh lý đi kèm (chỉ là tên chất/enzyme/thuốc/test) → **TÊN_XÉT_NGHIỆM** hoặc **THUỐC** tùy ngữ cảnh.
- Quy tắc áp dụng cho MỌI bệnh, không chỉ một bệnh cụ thể.

**B. Disease vs Symptom**:
- "Bệnh nhân MẮC bệnh này" → CHẨN_ĐOÁN
- "Bệnh nhân CẢM NHẬN được cái này" → TRIỆU_CHỨNG
- Test câu vàng: "Bệnh nhân có bị bệnh này không?" (Yes → CHẨN_ĐOÁN, No → TRIỆU_CHỨNG/khác)

**C. Compound Disease KHÔNG tách**:
- "X Y" là một bệnh/tổn thương thống nhất → giữ NGUYÊN VẸN
- KHÔNG tách "viêm X" → ["viêm", "X"]
- KHÔNG tách "ung thư X" → ["ung thư", "X"]
- Có thể dùng regex pattern `^\w+ (?:organ|symptommodifier)$` để detect compound

**D. Negation Context**:
- "không X", "chưa X", "âm tính X", "loại trừ X", "phủ nhận X" trước entity (window 30 chars, không vượt `.\n`) → **isNegated** = true.

**E. Family / Historical Context**:
- "Tiền sử:", "Trước đây:", "Cách đây X năm", "Bố/Mẹ/Ông/Bà [bị/có] X", "Tiền sử gia đình [có/bị] X" → **isHistorical** (nếu nói về quá khứ của bệnh nhân) hoặc **isFamily** (nếu nói về người nhà).

**F. Position Validation**:
- Với MỖI entity, kiểm tra: `input_text[position[0]:position[1]] == entity_text` (case-sensitive).
- Nếu khác → position sai → cố gắng tìm lại vị trí đúng. Nếu không tìm được → DROP.

# 🧠 R39 — TIÊU CHÍ GOLD STANDARD 7 CÂU HỎI (ÁP DỤNG MỌI ENTITY):

Với MỖI mention, **bắt buộc** trả lời 7 câu hỏi theo thứ tự:

1. **Text hợp lệ?** `input_text[pos[0]:pos[1]] == text`? NO → DROP.
2. **Là concept y khoa?** Có thuộc 1 trong 5 type không? NO → DROP.
3. **Compound hay bare?** Có verb bệnh lý trước không? → CHẨN_ĐOÁN / TÊN_XN.
4. **Disease hay symptom?** Bệnh nhân mắc hay cảm nhận? → CHẨN_ĐOÁN / TRIỆU_CHỨNG.
5. **Drug cụ thể hay class?** Class generic thì DROP.
6. **Assertion context?** isNegated/isFamily/isHistorical triggered?
7. **Sub-span của entity lớn?** Nếu có → DROP (giữ entity cha).

→ Trả lời 7 câu hỏi này trước khi quyết định type và assertions.
...
# 🛡️ R39-SELFVERIFY — TỰ KIỂM TRA SAU KHI CLASSIFY (BẮT BUỘC):

Trước khi return output, kiểm tra 5 ràng buộc:
1. **Position match**: Mỗi entity có `input_text[pos[0]:pos[1]] == text` không? NO → DROP.
2. **Type hợp lệ**: type ∈ {THUỐC, CHẨN_ĐOÁN, TRIỆU_CHỨNG, TÊN_XÉT_NGHIỆM, KẾT_QUẢ_XÉT_NGHIỆM}. NO → fix.
3. **Assertion đúng**: assertions ∈ {[], [isNegated], [isFamily], [isHistorical], combinations}. NO → fix.
4. **No hallucinated candidates**: `candidates` chỉ chứa ICD/RxNorm codes chuẩn. NO invented codes.
5. **No overlap với Stage 1 positions**: nếu 2 entities text gần giống, dedup bớt giữ 1.

→ Nếu fail → sửa TRƯỚC KHI return. KHÔNG skip self-verify.

# ⛔ R39 (2026-07-24) — 4 QUY TẮC LỌC NHIỄU KHI CLASSIFY

1. **CHATBOT ARTIFACT → DROP**:
   - "Cảm ơn bạn...", "Hy vọng...", "Nếu bạn có thêm câu hỏi...", "Xin chào", "Chúc bạn..." → KHÔNG phân loại → trả về type "TRIỆU_CHỨNG" hoặc DROP (đánh dấu type=null nếu format cho phép).

2. **NARRATIVE CÂU DÀI > 80 chars → KIỂM TRA KỸ**:
   - Thường là LLM extract cả câu. Nếu có dấu chấm `,.;:` bên trong → gần như chắc chắn narrative → DROP (type=null).
   - Lý do: text quá dài sẽ WER cao + Jaccard penalty hallucination.

3. **KIỂM TRA type-error nguy hiểm**:
   - "Buồn nôn" (lowercase b) trong input có "B" viết hoa → giữ text gốc "Buồn nôn" trong input, KHÔNG ép lowercase.
   - "ảo giác xuất hiện" trong input thực tế là "ảo giácxuất hiện" (dính từ) → đây là text hallucination, vẫn giữ text Stage 1 trả về.
   - Nhưng position PHẢI match input. Nếu input[start:end] != text → set type=null (DROP).

4. **DRUG BRAND NAME OVERRIDE**:
   - Brand name (Crestor, Tylenol, Panadol, Augmentin, Ventolin, Nexium, Lipitor, Plavix, Zocor, Lasix, Zofran, Zithromax, Glucophage, Combivent, Voltaren, Toradol, Advil) → THUỐC (KHÔNG cần match trong dict nếu đã rõ là brand).

# 🎯 R39 — NGUYÊN TẮC CLASSIFY THEO PATTERN (GENERIC):

Khi classify một entity, dựa vào **PATTERN** của nó (KHÔNG liệt kê cụ thể bệnh nào):

| Pattern của entity | Type | Áp dụng khi |
|---|---|---|
| Compound viêm X (X = organ) | CHẨN_ĐOÁN | Bất kỳ bệnh viêm nào + organ |
| Compound ung thư X (X = organ) | CHẨN_ĐOÁN | Bất kỳ bệnh ung thư nào + organ |
| Compound suy X (X = organ) | CHẨN_ĐOÁN | Bất kỳ bệnh suy giảm nào + organ |
| Compound gãy/thoát vị/phình X | CHẨN_ĐOÁN | Bất kỳ tổn thương cơ học nào |
| Named disease (proper noun + bệnh) | CHẨN_ĐOÁN | Bất kỳ bệnh có tên riêng (Kawasaki, Parkinson, Still, ...) |
| Tên bệnh viết tắt (NMCT, THA, ĐTĐ...) | CHẨN_ĐOÁN | Standard VN medical abbreviations |
| Common pediatric infections | CHẨN_ĐOÁN | Bệnh truyền nhiễm thường gặp ở trẻ |
| Common adult diseases | CHẨN_ĐOÁN | Theo pattern compound organ-disease |
| Enzyme/blood cell deficiency (có verb "thiếu") | CHẨN_ĐOÁN | "thiếu men X", "thiếu X" (X = blood/enzyme component) |
| Substance alone (G6PD, men X, enzyme Y) | TÊN_XÉT_NGHIỆM | Không có verb bệnh lý trước |
| Bacterial/viral/fungal infection | CHẨN_ĐOÁN | Có verb "nhiễm" rõ ràng |
| Triệu chứng đơn thuần | TRIỆU_CHỨNG | "đau X", "khó X", "sốt", "ho", "phù" |

# 🧠 CHAIN-OF-THOUGHT (CoT) - ĐỌC KỸ TRƯỚC KHI CLASSIFYVới MỖI mention, suy nghĩ theo 4 bước:
1. **Bản chất y khoa**: Đây là bệnh/tổn thương (CHẨN_ĐOÁN) hay triệu chứng BN cảm nhận (TRIỆU_CHỨNG) hay tên xét nghiệm (TÊN_XÉT_NGHIỆM) hay giá trị đo (KQ_XN) hay thuốc (THUỐC)?
2. **Câu hỏi vàng**: "Đây là thứ BỆNH NHÂN MẮC PHẢI hay thứ BỆNH NHÂN CẢM NHẬN ĐƯỢC?" → Bệnh mắc = CHẨN_ĐOÁN, Cảm nhận = TRIỆU_CHỨNG.
3. **Test name hay body part?**: "Đây là tên xét nghiệm/thủ thuật hay chỉ là tên cơ quan?" → Test = TÊN_XÉT_NGHIỆM, Cơ quan alone = DROP/KQ_XN.
4. **Assertion**: Có phải tiền sử (isHistorical) hay phủ định (isNegated) hay gia đình (isFamily) không?

# 5 LOẠI ENTITY
- type: BẮT BUỘC chọn đúng 1 trong 5 loại:
  - **THUỐC**: Tên thuốc, hoạt chất, liều lượng (`aspirin 325mg po bid`, `Crestor 20mg x 1`).
  - **CHẨN_ĐOÁN**: Tên bệnh lý, hội chứng, tổn thương hình ảnh, bất thường ECG, viết tắt, dấu hiệu lâm sàng bất thường (`tăng huyết áp`, `THA`, `đái tháo đường`, `ĐTĐ`, `nhồi máu cơ tim`, `NMCT`, `RLLL`, `COPD`, `CKD`, `ngoại tâm thu nhĩ`, `ST chênh lên`, `thâm nhiễm phổi`, `xẹp phổi`).
    - ⚠️ **QUAN TRỌNG**: "viêm X", "suy X", "ung thư X", "thoát vị X", "gãy X", "nhiễm X", "rối loạn X" → CHẨN_ĐOÁN (compound disease, KHÔNG tách).
    - ⚠️ **Tổn thương / bất thường hình ảnh**: `xẹp phổi`, `tràn dịch màng phổi`, `tim to`, `hở van hai lá`, `gãy xương` → CHẨN_ĐOÁN.
    - ⚠️ **Bất thường xét nghiệm**: `thiếu máu tan huyết`, `suy thận cấp`, `nhiễm khuẩn huyết` → CHẨN_ĐOÁN (không phải triệu chứng).
  - **TRIỆU_CHỨNG**: Biểu hiện cơ năng/thực thể BỆNH NHÂN CẢM NHẬN ĐƯỢC (`đau ngực`, `khó thở`, `sốt`, `ho`, `nôn`, `buồn nôn`, `mệt mỏi`, `chóng mặt`, `đau đầu`).
...
# 🚨 R38 (2026-07-22) — 8 TRƯỜNG HỢP PHÂN LOẠI DỄ SAI (ĐỌC KỸ TRƯỚC KHI GÁN TYPE)

**Câu hỏi VÀNG trước khi classify: "Đây là cái GÌ trong bệnh sử — bệnh, triệu chứng, thuốc, hay chỉ là mô tả cơ chế/trigger?"**

**1. Nhiễm trùng / Nhiễm khuẩn → CHẨN_ĐOÁN** (KHÔNG phải TRIỆU_CHỨNG):
   - `nhiễm khuẩn`, `nhiễm virus`, `nhiễm trùng`, `nhiễm vi khuẩn`, `nhiễm nấm` → **CHẨN_ĐOÁN**
   - Lý do: Là bệnh có mã ICD, dù đứng trong list "cần tránh". Câu trả lời "BN có CẢM NHẬN nhiễm khuẩn không?" → KHÔNG → không phải TRIỆU_CHỨNG.

**2. Vitamin → THUỐC** (KHÔNG phải TÊN_XÉT_NGHIỆM):
   - `Vitamin K`, `Vitamin A`, `Vitamin B12`, `Vitamin D`, `vitamin C` → **THUỐC**
   - Lý do: Là supplement/drug có trong RxNorm. Vitamin dùng trong "điều trị" hoặc "tránh" → là thuốc, không phải chỉ định CLS.

**3. Pattern `bị <disease>` → CHẨN_ĐOÁN** (KHÔNG phải TRIỆU_CHỨNG):
   - `trẻ bị thiếu men G6PD`, `bệnh nhân bị viêm phổi`, `bị suy tim`, `bị hen`, `bị tăng huyết áp`
   - → **CHẨN_ĐOÁN** (kể cả khi có subject phía trước như "trẻ", "bệnh nhân", "bệnh nhi").
   - Lý do: "bị X" trong tiếng Việt là cấu trúc "subject MẮC disease" → disease đó là CHẨN_ĐOÁN.

**4. Cells / chemical processes → KHÔNG phải TRIỆU_CHỨNG** (chỉ là mô tả cơ chế):
   - `hồng cầu`, `bạch cầu`, `tiểu cầu` → KHÔNG extract (BN không cảm nhận được)
   - `oxy hóa`, `khử oxy`, `chuyển hóa`, `đông máu`, `tan huyết`, `phá hủy` → KHÔNG extract
   - Test: "BN có CẢM NHẬN được điều này không?" → KHÔNG → DROP.

**5. Trigger substances → KHÔNG phải TRIỆU_CHỨNG** (chỉ là tác nhân gây bệnh):
   - `đậu tằm`, `băng phiến` (long não), `hóa chất`, `thực phẩm` (đứng riêng) → KHÔNG extract
   - Test: "Cái này GÂY RA bệnh hay là BIỂU HIỆN bệnh?" → Gây ra → DROP.

**6. Generic categories / actions / body sites → KHÔNG phải TÊN_XÉT_NGHIỆM**:
   - `thực phẩm`, `hóa chất`, `thuốc` (đứng riêng) → DROP (generic category, không phải test cụ thể)
   - `phân tích`, `lấy máu`, `kiểm tra`, `thu thập` → DROP (chỉ là actions)
   - `gót chân`, `gót chân trẻ`, `máu khô`, `tĩnh mạch` → DROP (body sites, không phải test names)
   - Test: "Đây là TÊN 1 test cụ thể hay chỉ là DIỄN TẢ cách làm test?" → Diễn tả → DROP.

**7. Narrative consumption / action phrases → KHÔNG phải entity**:
   - `ăn đậu tằm`, `ăn uống bình thường`, `tiếp xúc với băng phiến`, `sử dụng thuốc`
   - `dùng thuốc nam`, `uống thuốc`, `hạ sốt` (hành động điều trị) → DROP
   - Test: "Đây là HÀNH ĐỘNG (verb phrase) hay THỰC THỂ (noun)?" → Hành động → DROP.

**8. Body part đơn lẻ + descriptive fragments → DROP**:
   - Body part alone: `ngực`, `bụng`, `đầu`, `lưng` → DROP (đã có ở R35)
   - Fragments: `thiếu men này`, `thiếu máu` (đứng riêng), `dễ bị phá hủy`, `hồng cầu trở nên mong manh`, `có tính oxy hóa cao` → DROP
   - Test: "Cụm này MÔ TẢ đầy đủ 1 entity hay chỉ là MẢNH của câu dài?" → Mảnh → DROP.

🔥 **CHECKLIST TRƯỚC KHI CLASSIFY MỖI MENTION**:
□ Nếu mention là tên bệnh có ICD → CHẨN_ĐOÁN.
□ Nếu mention là triệu chứng BN cảm nhận được → TRIỆU_CHỨNG.
□ Nếu mention là tên thuốc cụ thể (generic/brand) → THUỐC.
□ Nếu mention là tên chỉ định CLS cụ thể → TÊN_XÉT_NGHIỆM.
□ Nếu mention là giá trị đo / kết quả → KẾT_QUẢ_XÉT_NGHIỆM.

# 🚨 R38 (2026-07-22) — BẢNG PHÂN LOẠI TYPE QUAN TRỌNG (BẮT BUỘC THEO)

| Entity text pattern | Type | Lý do |
|---------------------|------|-------|
| `Thiếu men G6PD`, `Thiếu máu do tan huyết`, `Thiếu máu tan huyết` | **CHẨN_ĐOÁN** | Bệnh lý về máu |
| `suy thận cấp`, `suy tim`, `suy gan`, `suy hô hấp` | **CHẨN_ĐOÁN** | Bệnh lý nặng (organ failure) |
| `bại não`, `chậm phát triển trí tuệ`, `rối loạn vận động` | **CHẨN_ĐOÁN** | Tổn thương thần kinh / abnormal findings |
| `nhiễm khuẩn`, `nhiễm virus`, `nhiễm trùng` | **CHẨN_ĐOÁN** | Bệnh nhiễm trùng |
| `nhồi máu cơ tim`, `nhồi máu não`, `xuất huyết não` | **CHẨN_ĐOÁN** | Bệnh lý nặng |
| `Glucose-6-Phosphate Dehydrogenase` (enzyme name alone) | **TÊN_XÉT_NGHIỆM** | Tên enzyme → chỉ định test |
| `men G6PD` alone | **TÊN_XÉT_NGHIỆM** | Tên enzyme → chỉ định test |
| `đau ngực`, `khó thở`, `sốt`, `ho` | **TRIỆU_CHỨNG** | BN cảm nhận được |
| `Sốt cao`, `Khó thở nhẹ`, `Đau ngực trái` | **TRIỆU_CHỨNG** | Triệu chứng (có qualifier) |
| `Hồng cầu`, `Bạch cầu`, `Tiểu cầu` (alone) | **DROP** | Cell, BN không cảm nhận |
| `Oxy hóa`, `Tan huyết`, `Phá hủy` (alone) | **DROP** | Chemical process |
| `Thuốc`, `Thực phẩm`, `Hóa chất` (alone) | **DROP** | Generic category |
| `Thuốc hạ sốt`, `Kháng sinh`, `Corticoid` | **DROP** | Drug-class generic |
| `Ăn đậu tằm`, `Tiếp xúc với băng phiến`, `Hạ sốt` | **DROP** | Narrative action |
| `Thiếu men này`, `dễ bị phá hủy`, `mong manh` | **DROP** | Fragment |
| `Trẻ sơ sinh`, `Người lớn` | **DROP** | Age descriptor |
| `Phân tích`, `Chẩn đoán`, `Sàng lọc sớm` | **DROP** | Generic action |
| `Gót chân trẻ`, `Máu khô` | **DROP** | Sample type alone |
| `Mẹ đang cho con bú` | **DROP** | Narrative |

# 🔑 CÂU HỎI VÀNG CHO MỖI MENTION (LLM phải tự hỏi):
1. **"BN MẮC BỆNH này hay CHỈ CẢM NHẬN?"** → Mắc = CHẨN_ĐOÁN, Cảm nhận = TRIỆU_CHỨNG.
2. **"Đây là TÊN TEST cụ thể hay ACTION/DESCRIPTION?"** → Test cụ thể = TÊN_XÉT_NGHIỆM, Action = DROP.
3. **"Đây là DRUG CLASS hay SPECIFIC DRUG?"** → Class = DROP, Specific = THUỐC.
4. **"Đây là CELL/PROCESS hay ENTITY?"** → Cell/Process alone = DROP.

# ⚠️ QUAN TRỌNG: Nếu KHÔNG chắc chắn type, vẫn trả entry với type GẦN NHẤT (để Python xử lý drop ở post-process nếu cần). KHÔNG ĐƯỢC BỎ SÓT mention.
...
- assertions: Mảng chuỗi các nhãn ngữ cảnh lâm sàng, BẮT BUỘC kiểm tra kỹ từng nhãn (có thể kết hợp nhiều nhãn nếu phù hợp):
  - "isNegated": Nếu mention bị phủ định bởi các từ khóa `không`, `chưa`, `âm tính`, `chưa ghi nhận`, `không thấy`, `loại trừ`. LƯU Ý QUAN TRỌNG: Nếu một câu có chuỗi nhiều triệu chứng phủ định ngăn cách bởi dấu phẩy (`không ho, sốt, hay khó thở`), thì CẢ 3 entity `ho`, `sốt`, `khó thở` ĐỀU BẮT BUỘC gán `["isNegated"]`. Tuy nhiên, các kết quả xét nghiệm bình thường (`ECG bình thường`, `nhịp xoang đều`) KHÔNG bị phủ định (để mảng rỗng `[]`).
    - **🚨 R38 (2026-07-22) — TRÁNH FALSE isNegated từ "không" ở CÂU TRƯỚC**:
      - "không" chỉ negate trong CÙNG MỆNH ĐỀ (clause), KHÔNG lan sang câu sau khi có dấu phẩy/chấm.
      - VD SAI: `"Nếu không được điều trị..., tình trạng thiếu máu kéo dài kèm vàng da sơ sinh sẽ trở thành vấn đề nghiêm trọng"` → `vàng da sơ sinh` KHÔNG bị negate (mệnh đề mới sau dấu phẩy).
      - VD ĐÚNG: `"Không ho, không sốt, không khó thở"` → cả 3 đều `["isNegated"]` (cùng mệnh đề).
      - VD ĐÚNG: `"Không dùng long não, băng phiến"` → cả 2 đều `["isNegated"]` (cùng mệnh đề với verb "dùng").
...
# 🚨 R39 (2026-07-24) — BẢNG LOẠI TRỪ CỨNG (HARD REJECTION RULES):

| Pattern trong text | ICD code PHẢI LOẠI | Lý do (GENERIC PRINCIPLE) |
|---|---|---|
| Blood disease + acquired (thiếu máu, hemophilia, etc.) | Congenital Q-codes | Blood disease KHÔNG phải dị tật bẩm sinh → loại Q00-Q99 |
| Enzyme deficiency + acquired | Chapter Q + chapter E vitamin | Enzyme deficiency → D55-D58; KHÔNG testis defect / vitamin |
| Cardiac inflammatory (viêm cơ tim, viêm màng tim) | Chapter B viral | Myocarditis → I40-I41; KHÔNG B33.2 viral pericarditis |
| Vascular dilation (aneurysm phình X) | Other organ chapter | Coronary aneurysm → I25.4; KHÔNG intracranial/intra-abdominal |
| Adult rheumatism | Juvenile M08 | Adult RA → M05-M06 |
| Active disease | Factor Z00-Z99 | Active disease → disease chapter, KHÔNG screening/factor |
| Named disease (Kawasaki, ...) | Generic organ code | Named disease có specific code (vd Kawasaki → M30.3) |
| Vasculitis (any) | Bacterial inflammation | Vasculitis → I77/M30; KHÔNG general inflammatory code |

# 🚨 R39 — RXNORM HARD REJECTION (GENERIC):
| Context pattern | Action | Lý do |
|---|---|---|
| Substance name alone in lab context (bicarbonate, sodium, ... as measured values) | DROP candidate | Lab values ≠ drugs to treat |
| "kháng X" trong resistance context (vd "kháng methicillin/X" mô tả resistance) | DROP candidate | Resistance mention ≠ drug treatment |
| Drug class name as entity (kháng sinh, NSAID, corticoid, ...) | DROP candidate | Class name, not specific drug |
| Vitamin generic without brand | DROP candidate | Vitamin generic không có rxcui specific |
...
3. **ĐỐI CHIẾU từng candidate với context**:
   - "Code này mô tả đúng concept của entity không?"
   - "Có qualifier nào trong entity text mà candidate chưa cover?"
   - "Có candidate nào SAI concept không (vd Q55 testis defect cho G6PD anemia)?"
4. **CHẤM ĐIỂM** mỗi candidate từ 1-10 dựa trên độ phù hợp với entity + context:
