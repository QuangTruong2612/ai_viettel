...
class LLMConfig:
    ...
    # LM Studio user thường mount server ở :1234 v1; endpoint chuẩn là
    # /v1/chat/completions nhưng OpenAI client tự gắn path khi base_url đã có /v1.
    base_url: str = "http://127.0.0.1:1234/v1"
    api_key: str = "lm-studio"  # LM Studio chấp nhận bất kỳ
    model: str = "qwen2.5-7b-instruct"  # model người dùng đang load trong LM Studio
    temperature: float = 0.0
    top_p: float = 1.0

    # max_tokens=1024 đủ chứa JSON ~20 entities (~80 chars mỗi).
    ...
    def from_env(cls) -> "LLMConfig":
        ...
        return cls(
            base_url=chosen_url,
            api_key=os.environ.get("OLLAMA_API_KEY") or os.environ.get("LMSTUDIO_API_KEY", cls.api_key),
            model=os.environ.get("OLLAMA_MODEL") or os.environ.get("LMSTUDIO_MODEL", cls.model),
